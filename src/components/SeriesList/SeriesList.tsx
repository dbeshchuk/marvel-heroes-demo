...
const SeriesList = () => {
  ...
  const getHeroesSeries = async (id: string) => {
    setLoading(true)

    await marvelAPI
      .fetchHeroSeries(id, offset)
      .then((response) => {
  ...
  const updateHeroesSeries = async (id : string) => {
    setLoading(true)
  ...
  useEffect(() => {
    getHeroesSeries(heroId as string);
  }, [heroId]);

  useEffect(() => {
    updateHeroesSeries(heroId as string);
  }, [offset]);
